...
{
  //check to enable or disable 'Create Share' button, 
  //this button will be enabled if there is any text in textfield
  if(identity.split(' ').join('').length > 0){
     $('#create_share_button').removeAttr('disabled');
  } else {
      $('#create_share_button').attr('disabled','disabled');
  }

  if ( identity.indexOf(" ") > 0 || identity.indexOf(",") > 0) {
    
    //write new block to toggle which form element is displayed, 
    //carry over values, register/unregister event listeners
...
$(document).ready(function(){
    // disable 'Create Share' button on load
    $('#create_share_button').attr('disabled','disabled');
});
